 ...
 */
public class FondoReciboDAO extends GenericDAO {

    public List<FondoRecibo> getAllFondoRecibosActivos() {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Criteria criteria = session.createCriteria(FondoRecibo.class);
        criteria.add(Restrictions.eq("activo", true));
        return (List<FondoRecibo>) criteria.list();
    }
    
    public List<FondoRecibo> getAllFondoRecibosInactivos() {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Criteria criteria = session.createCriteria(FondoRecibo.class);
    ...
    public String getFondoReciboByTitular(TitularCuit titular) {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Criteria criteria = session.createCriteria(FondoRecibo.class);
        criteria.add(Restrictions.eq("id", titular));
        FondoRecibo fr = (FondoRecibo) criteria.uniqueResult();
        return fr.getUbicacion();
    }

    public FondoRecibo getFondoReciboByTitular2(TitularCuit titular) {
        Session session = HibernateUtils.getSessionFactory().getCurrentSession();
        Criteria criteria = session.createCriteria(FondoRecibo.class);
    ...
    public Integer getCodigoSiguiente() {
        ...
        fondos = (List<FondoRecibo>) criteria.list();
        if (fondos != null && !fondos.isEmpty()) {
            FondoRecibo cons = fondos.get(0);
...
//        criteria.add(Restrictions.eq("activo", true));
//        criteria.add(Restrictions.eq("cuit", "00-00000000-0"));
////        criteria.add(Restrictions.eq("master", 0));
