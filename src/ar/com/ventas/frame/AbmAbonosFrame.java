...
public class AbmAbonosFrame extends javax.swing.JFrame {
     ...
     * @param row
     */
    public AbmAbonosFrame(Integer rowR, Integer rowA, Integer row, List<Abono> abonos) {
        initComponents();
...
//        setExtendedState(6); // this.MAXIMIZED_BOTH
    ...
    private void initComponents() {
        ...
        tabla.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "CONSORCIO", "FRECUENCIA", "PER", "CUOTA", "CUOTAS", "VALOR CUOTA", "TOTAL ABONO", "PEND.FACTURAR", "ADMIN"
            }
        ) {
    ...
    private void comboRActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_comboRActionPerformed
        int rowRn = comboR.getSelectedIndex();
        if (rowRn > 0) {
            Rubro ru = rubros.get(rowRn - 1);
            generaLista(ru);
        }
    ...
    private void noRenovarBtnActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_noRenovarBtnActionPerformed
        ...
        try {
            comprobante = new ComprobanteService().getUltimoComprobanteByConsorcioAndRubro(cons, ru);
        } catch (Exception ex) {
            Logger.getLogger(AbmAbonosFrame.class.getName()).log(Level.SEVERE, null, ex);
            JOptionPane.showMessageDialog(this, "ERROR 287 - COMPROBANTES");
            return;
        }
        if(comprobante.getOriginal()){
            JOptionPane.showMessageDialog(this, "VERIFIQUE CONSORCIO ORIGINAL");
            return;
        }
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(AbmAbonosFrame.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new AbmAbonosFrame(null, null, null, null).setVisible(true);
            }
        });
    }
    ...
    private void modificar() {
        int rown = tabla.getSelectedRow();
        if (rown < 0) {
            JOptionPane.showMessageDialog(this, "DEBE SELECCIONAR UN ABONO PARA MODIFICAR");
            return;
        }
        Abono a = abonos.get(rown);
        rowA = comboA.getSelectedIndex();
        rowR = comboR.getSelectedIndex();
        ModificarAbonoFrame maf = new ModificarAbonoFrame(a, rowR, rowA, rown, abonos);
        maf.setVisible(true);
        this.dispose();
    }
    ...
    private void cancelados() {
        AbmAbonosCanceladosFrame aacf = new AbmAbonosCanceladosFrame(rowR, rowA, row, abonos);
        aacf.setVisible(true);
        this.dispose();
    }

    private void generaLista(Rubro ru) {
...
//            Logger.getLogger(AbmAbonosFrame.class.getName()).log(Level.SEVERE, null, ex);
    ...
    private void excel() {
        String ruta = "C:/alfa_sistema/data/excel/abonos.xls";
        ...
        for (Abono ab : abonos) {
            ...
            if (ab.getRubro().getEsAbono()) {
                if (r.getId().equals(2L)) {
                    cuando = ab.getCuotaFacturada().toString();
                } else {
                    if (ab.getTextoPeriodo().equals(0)) {
                        cuando = UtilFrame.mesAnteriorEnLetras(ab.getFechaPeriodo());
                    } else {
                        cuando = UtilFrame.mesActualEnLetras(ab.getFechaPeriodo());
                    }
                }
            }
            ...
            ob[5] = df.format(cuo);
            ob[6] = df.format(ab.getImporte());
            ob[7] = pend;
            ob[8] = ab.getConsorcio().getAdministrador().getRazonSocial();
            tbl.addRow(ob);
        }
        tabla.setModel(tbl);
    }
